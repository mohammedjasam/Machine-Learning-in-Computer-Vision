function [] = Scene_pixel_classification( testImageList )
...
%size(file_names)
for j = 1:size(testImageList, 1)
    testFile = testImageList(j);
...
for zz = 1:size(testImageList, 1)
    testFile = testImageList{zz, 1};
    ...
    opfilepath = sprintf('output\\%s-k=(%d-other %d-sky %d-buildg), features used = (%s)', testFile, K_O, K_S, K_B, feature_used);
