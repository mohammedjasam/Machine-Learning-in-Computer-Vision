...
M
noDataPts = 0;
if strcmpi(mode, 'hsvycbcrFeature') == 1
    disp('Using hsv ycbcr mode');
    noDataPts = 6*NData;
    sprintf('M:%d --- 6M:%d', M, noDataPts)
else
    noDataPts = NData;
end
...
K = 38;%M;%Change K value accordingly 
K
%
%STEP-1%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:no_of_indiv
    ...
    for j = 3:no_files_in_dir
        ...
        img_ycbcr = rgb2ycbcr(imrgb);
        %img_grayscale = img_hsv;%img_hsv(:, :, 2);
        IM(:,counter) = [img_hsv(:)' img_ycbcr(:)']; %Accessing i'th column and putting the i'th image grayscale values as a column        %figure();
...
for i = 1:no_of_indiv
    ...
    for j = 3:no_files_in_dir
        ...
        img_ycbcr=rgb2ycbcr(imrgb);
        %img_grayscale = img_hsv;%img_hsv(:, :, 2);
        IMTest(:,testCounter) = [img_hsv(:)' img_ycbcr(:)' ]; %Accessing i'th column and putting the i'th image grayscale values as a column
...
for i = 1:TSize
    for j = 1:M   
      temp=  [(AlphaTest(:,i) - Alpha(:,j))];
        dist = 0;
        for k = 1:K;
            dist = dist + temp(k).^2; %This calculates Norm2
           % dist = dist+temp(k);  %This calculates nNorm1
        end
        dist = sqrt(dist);
...
fprintf('end of the program: ycbcr, hsv - 6feature - accuracy percentage:%f', (accuracy/TSize*100));
