...
M
noDataPts = 0;
if strcmpi(mode, 'rgbhsvFeature') == 1
    disp('Using rgb hsv mode');
    noDataPts = 6*NData;
...
K = 150;%M;%Change K value accordingly 
K
%
%STEP-1%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:no_of_indiv
    ...
    for j = 3:no_files_in_dir
        ...
        IM(:,counter) = [imrgb(:)' img_hsv(:)' ]; %Accessing i'th column and putting the i'th image grayscale values as a column
        %figure();
...
for i = 1:no_of_indiv
    ...
    for j = 3:no_files_in_dir
        ...
        img_hsv = rgb2hsv(imrgb);
        %img_grayscale = img_hsv;%img_hsv(:, :, 2);
        IMTest(:,testCounter) = [imrgb(:)' img_hsv(:)' ]; %Accessing i'th column and putting the i'th image grayscale values as a column
...
fprintf('end of the program: rgb, hsv - 6feature - accuracy percentage:%f', (accuracy/TSize*100));
