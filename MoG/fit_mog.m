...
function [lambda, mu, sig] = fit_mog (x, K, precision)
    ...
    while true
        ...
        %the following is visualization
        [XX,YY] = meshgrid (-10:0.01:10,-10:0.01:10);
        x_y_matrix = [XX(:) YY(:)]; n = size(XX,1);        
%         figure(100); clf;
%         scatter(x(:,1), x(:,2), 40, r, '.');
%         axis([-6,6,-1,9]);
...
%         hold on;
        for k=1:K
            ...
            tempk = mvnpdf (x_y_matrix, mu(k,:), sig{k});
            gaussiank = reshape (tempk, n, n);

            % Create the contour plot.
            contour(XX,YY,gaussiank,4);
        end
        hold off;
        drawnow;
